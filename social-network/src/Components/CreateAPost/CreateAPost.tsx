...
export default function CreateAPost(props: any) {
	...
	const handlePostSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		FD.append('group', postData.post.group as string);
		FD.append('imgUpload', postData.post.imgUpload as string);
	...
	return (
		...
		<div className="mt-4 mx-6 mb-0 p-6 bg-blue-50 rounded-xl">
			<form onSubmit={(e) => handlePostSubmit} name="post" className="flex flex-col [&>input]:mb-2 [&>textarea]:mb-2 [&>*]:outline-none">
				<input
					type="title"
					name="title"
					onChange={(e) => {
				...
				<div className="flex flex-rows justify-between gap-6 [&>*]:outline-none">
					<input
						...
						required
					></input>
					<label htmlFor="imgUpload" className="my-auto cursor-pointer">
						<img
							id=""
							src=""
					...
					/>
					<input type="submit" id="submitBtn" value="Submit" className="btn-custom font-semibold">
					</input>
				</div>
			</form >
		</div >
